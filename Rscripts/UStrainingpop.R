...
# 8.0 Define two options for Genomic Prediciton in BGLR 
  ...
  # 8.4 Fill first column with Names of training and validation populations  
    Train.Valid.Names <- c("C2.US Train and C2.SK Valid",
                           "C2.US Train and C2.GRIN Valid",
                           "C2.US Train and OH.US Valid",
                           "C2.US LOOCV")
    Output.data[1:4,1] <- Train.Valid.Names 
    
#####################################################################################      
# Section 9.0: C2.US panel as training population for C2.SK panel
  # 9.1 Start a loop so ten iterations can be completed
    C2.US.Train.SK.valid <- lapply(1:10, function(x){
  # 9.2 Filter the Phenotype file to find PIs assigned to C2.US panel
      C2.US.Y <- filter(Pheno_Popped, Population == "C2.US")
  # 9.3 Find those in the C2.SK panel
        C2.SK.Y <- filter(Pheno_Popped, Population == "C2.SK")
  # 9.4 Make object without C2.SK.Y ISW BLUEs
          C2.SK.NA <- C2.SK.Y
            C2.SK.NA$ISW <- NA
  # 9.5 Merge dataframes  
              Exp.Y <- rbind(C2.US.Y, C2.SK.NA)
  ...
  # 9.10 Extract predicted phenotypes for C2.SK panel
                    dat <- fm$yHat # extracts predicted phenotypes from BGLR output
                  dat <- dat[(length(C2.US.Y$taxa)+1):length(dat)] # subset to only consider those for the C2.SK panel
  ...
  # 9.14 unlist and convert to numeric so I can calculate average  
    C2.US.Train.SK.valid <- as.numeric(unlist(C2.US.Train.SK.valid))
  # 9.15  Add average to output dataframe and the standard deviation
    Output.data[1,2] <- mean(C2.US.Train.SK.valid)
    Output.data[1,3] <- sd(C2.US.Train.SK.valid)     
#####################################################################################      
# Section 10.0: C2.US panel as training population for C2.GRIN panel  
  ...
  # 10.10 Extract predicted phenotypes for C2.GRIN panel
    dat <- fm$yHat # extracts predicted phenotypes from BGLR output
    dat <- dat[(length(C2.US.Y$taxa)+1):length(dat)] # subset to only consider those for the C2.GRIN panel
  ...
  # 10.14 unlist and convert to numeric so I can calculate average  
    C2.US.Train.GRIN.valid <- as.numeric(unlist(C2.US.Train.GRIN.valid))
  # 10.15  Add average to output dataframe and the standard deviation
    Output.data[2,2] <- mean(C2.US.Train.GRIN.valid)
    Output.data[2,3] <- sd(C2.US.Train.GRIN.valid)    
#####################################################################################      
# Section 11.0: C2.US panel as training population for OH.US panel  
  # 11.1 Start a loop to run 10 iterations
    C2.US.Train.OH.US.valid <- lapply(1:10, function(x){
  ...
  # 11.10 Extract predicted phenotypes for OH.US panel
    dat <- fm$yHat # extracts predicted phenotypes from BGLR output
    dat <- dat[(length(C2.US.Y$taxa)+1):length(dat)] # subset to only consider those for the OH.US panel
  ...
  # 11.14 unlist and convert to numeric so I can calculate average  
    C2.US.Train.OH.US.valid <- as.numeric(unlist(C2.US.Train.OH.US.valid))
  # 11.15  Add average to output dataframe and the standard deviation
    Output.data[3,2] <- mean(C2.US.Train.OH.US.valid)
    Output.data[3,3] <- sd(C2.US.Train.OH.US.valid)       
#####################################################################################      
# Section 12.0: Leave-one-out Cross-validation within C2.US panel 
  # 12.1 Filter the Phenotype file to find PIs assigned to C2.US panel
    C2.US.Y <- filter(Pheno_Popped, Population == "C2.US") 
  # 12.2 Subset genotype data to just C2.US
    Exp.G <- subset(Geno, taxa %in% C2.US.Y$taxa)
  # 12.3 Remove "Taxa" column from genotype file
    Exp.G <- Exp.G[,-1]
  # 12.4 Subset phenotype data to just C2.US
    Exp.Y <- C2.US.Y
...
# Section 13.0 write output to csv file   
  ...
  # 13.2 write to csv file
    write.csv(x=Output.data, file="C2.US.Trainer.csv", row.names=F)   
  # 13.3 Prepare for next round analysis     
    rm(list= c("C2.US.Y", "Output.data"))    
    Output.data <- matrix(nrow=4, ncol=3)
  # 13.4 Fill first column with Names of training and validation populations  
    Train.Valid.Names <- c("OH.US Train and OH.SK Valid",
                         "OH.US Train and OH.GRIN Valid",
                         "OH.US Train and C2.US Valid",
                         "OH.US LOOCV")
    Output.data[1:4,1] <- Train.Valid.Names   
#####################################################################################      
# Section 14.0: OH.US panel as training population for OH.SK panel
  # 14.1 Start a loop to run 10 iterations
    OH.US.Train.OH.SK.valid <- lapply(1:10, function(x){
  ...
  # 14.4 Make object without OH.SK ISW BLUEs
    OH.SK.NA <- OH.SK.Y
    OH.SK.NA$ISW <- NA
  ...
  # 14.10 Extract predicted phenotypes for OH.SK panel
    dat <- fm$yHat # extracts predicted phenotypes from BGLR output
    dat <- dat[(length(OH.US.Y$taxa)+1):length(dat)] # subset to only consider those for the OH.US panel
  ...
  # 14.14 unlist and convert to numeric so I can calculate average  
    OH.US.Train.OH.SK.valid <- as.numeric(unlist(OH.US.Train.OH.SK.valid))
  # 14.15 Add average to output dataframe and the standard deviation
    Output.data[1,2] <- mean(OH.US.Train.OH.SK.valid)
    Output.data[1,3] <- sd(OH.US.Train.OH.SK.valid)    
#####################################################################################      
# Section 15.0: OH.US panel as training population for OH.GRIN panel  
  # 15.1 State a loop for 10 iterations
    OH.US.Train.OH.GRIN.valid <- lapply(1:10, function(x){
  # 15.2 Filter the Phenotype file to find PIs assigned to OH.US panel
    OH.US.Y <- filter(Pheno_Popped, Population == "OH.US")    
  # 15.3 Find those in the OH.GRIN panel
    OH.GRIN.Y <- filter(Pheno_Popped, Population == "OH.GRIN")
  # 15.4 Make object without OH.GRIN ISW BLUEs
    OH.GRIN.NA <- OH.GRIN.Y
    OH.GRIN.NA$ISW <- NA
  # 15.5 Merge dataframes  
    Exp.Y <- rbind(OH.US.Y, OH.GRIN.NA)
  ...
  # 15.9 Run BGLR genomic Prediction
    ETA<-list(list(X=Exp.G[,-1], model='BL'))
    fm <- BGLR(y=Exp.Y$ISW, ETA=ETA, nIter=nIter, burnIn=burnIn, verbose=FALSE)
  # 15.10 Extract predicted phenotypes for OH.GRIN panel
    dat <- fm$yHat # extracts predicted phenotypes from BGLR output
    dat <- dat[(length(OH.US.Y$taxa)+1):length(dat)] # subset to only consider those for the OH.GRIN panel
  ...
  # 15.14 unlist and convert to numeric so I can calculate average  
    OH.US.Train.OH.GRIN.valid <- as.numeric(unlist(OH.US.Train.OH.GRIN.valid))
  # 15.15 Add average to output dataframe and the standard deviation
    Output.data[2,2] <- mean(OH.US.Train.OH.GRIN.valid)
    Output.data[2,3] <- sd(OH.US.Train.OH.GRIN.valid)   
   
#####################################################################################      
# Section 16.0: OH.US panel as training population for C2.US panel  
  # 16.1 State loop for 10 iterations
    OH.US.Train.C2.US.valid  <- lapply(1:10, function(x){
  # 16.2 Filter the Phenotype file to find PIs assigned to OH.US panel
    OH.US.Y <- filter(Pheno_Popped, Population == "OH.US")    
  # 16.3 Find those in the C2.US panel
    C2.US.Y <- filter(Pheno_Popped, Population == "C2.US")
  # 16.4 Make object without C2.US.Y ISW BLUEs
    C2.US.NA <- C2.US.Y
    C2.US.NA$ISW <- NA
  # 16.5 Merge dataframes  
    Exp.Y <- rbind(OH.US.Y, C2.US.NA)
  ...
  # 16.9 Run BGLR genomic Prediction
    ETA<-list(list(X=Exp.G[,-1], model='BL'))
    fm <- BGLR(y=Exp.Y$ISW, ETA=ETA, nIter=nIter, burnIn=burnIn, verbose=FALSE)
  # 16.10 Extract predicted phenotypes for C2.US panel
    dat <- fm$yHat # extracts predicted phenotypes from BGLR output
    dat <- dat[(length(OH.US.Y$taxa)+1):length(dat)] # subset to only consider those for the C2.US panel
  ...
  # 16.14 unlist and convert to numeric so I can calculate average  
    OH.US.Train.C2.US.valid <- as.numeric(unlist(OH.US.Train.C2.US.valid))
  # 16.15 Add average to output dataframe and the standard deviation
    Output.data[3,2] <- mean(OH.US.Train.C2.US.valid)
    Output.data[3,3] <- sd(OH.US.Train.C2.US.valid)   
#####################################################################################      
# Section 17.0: Leave-one-out Cross-validation within OH.US panel 
  # 17.1 Filter the Phenotype file to find PIs assigned to OH.US panel
    OH.US.Y <- filter(Pheno_Popped, Population == "OH.US")
  # 17.2 Subset genotype data to just OH.US
    Exp.G <- subset(Geno, taxa %in% OH.US.Y$taxa)
  # 17.3 Remove "Taxa" column from genotype file
    Exp.G <- Exp.G[,-1]
  # 17.4 Subset phenotype data to just OH.US
    Exp.Y <- OH.US.Y
...
# Section 18.0 write output to csv file   
  ...
  # 18.2 write to csv file
    write.csv(x=Output.data, file="OH.US.Trainer.csv", row.names=F)      
  # :)    
